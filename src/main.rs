...
use a142886::{count, count_cell_centered, count_vertex_centered, Count};
use std::path::Path;
...
impl Center {
    ...
    fn term(self) -> fn(usize) -> Count {
        match self {
            Center::Cell => count_cell_centered,
            Center::Vertex => count_vertex_centered,
            Center::Both => count,
        }
    }
}
...
USAGE:
    a142886 [--max-n N] [--center cell|vertex|both] [--verify]
...
/// b-file). Returns the number of mismatches found.
