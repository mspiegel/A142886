...
/// Sharded variant of [`count_parallel`]: partition the bucket task list
/// across `n_pools` independent rayon pools of `pool_size` threads each.
/// Bypasses the per-pool depth-N Amdahl ceiling (~30× on this codebase)
/// on machines with significantly more than 30 cores — each pool's tail
/// is independent, so multiple pools' tails overlap rather than serialize.
///
/// **Semantics:** byte-identical to [`count_parallel`] for any
/// `(n_pools, pool_size)`. Buckets are independent by construction
/// (each slice is counted in exactly one bucket — DESIGN.md §4.2), so
/// summation across pools is commutative and the answer is invariant
/// to the partitioning.
///
/// **Partitioning:** stride (`tasks[i % n_pools]`). Cell buckets are
/// ordered by ax ascending then vertex by ax ascending in the source
/// task list, so stride distributes the heavy ax=1..6 cell plateau
/// across all pools rather than clumping them in one pool's class.
///
/// **Concurrency:** uses `std::thread::scope` to spawn one OS thread
/// per pool. Each OS thread calls `pool.install(|| par_iter().sum())`
/// which runs the closure on a worker thread of its own pool; the OS
/// thread blocks on result. Without `scope`, calling `pool.install`
/// sequentially on the main thread would serialize pool work.
///
/// **Use via CLI:** `--pools K --pool-size M`. Default path remains
/// [`count_parallel`].
pub fn count_parallel_sharded(n: usize, n_pools: usize, pool_size: usize) -> Count {
    if n == 0 {
        return 1;
    }
    if n % 4 == 2 || n % 4 == 3 {
    ...
    assert!(n_pools >= 1, "n_pools must be ≥ 1");
    assert!(pool_size >= 1, "pool_size must be ≥ 1");
    let n_u64 = n as u64;
    let xmax = n_u64 as i32;

    // Build the same task list as count_parallel.
    let mut tasks: Vec<(bool, i32)> = Vec::new();
    if n % 4 == 1 {
        tasks.push((false, 0)); // cell apex only
    } else {
    ...
    // Stride-partition into n_pools chunks.
    let chunks: Vec<Vec<(bool, i32)>> = (0..n_pools)
        .map(|i| {
            tasks
                .iter()
                .enumerate()
                .filter(|(idx, _)| idx % n_pools == i)
                .map(|(_, t)| *t)
                .collect()
        })
        .collect();

    // Build n_pools independent rayon pools.
    let pools: Vec<rayon::ThreadPool> = (0..n_pools)
        .map(|i| {
            rayon::ThreadPoolBuilder::new()
                .num_threads(pool_size)
                .thread_name(move |k| format!("rayon-{i}-{k}"))
                .build()
                .expect("rayon pool build")
        })
        .collect();

    // Concurrent dispatch via std::thread::scope: one OS thread per pool.
    std::thread::scope(|s| {
...
mod tests {
    ...
    /// Sharded oracle: `count_parallel_sharded` with K=1 (single pool)
    /// across several pool sizes must equal `count_parallel(n)` byte-for-
    /// byte. K=1 is the "no regression" gate — any divergence here is a
    /// bug in the sharding plumbing itself.
    #[test]
    fn sharded_k1_matches_parallel() {
        let ns: &[usize] = &[0, 4, 16, 32, 48, 60, 80, 100, 120];
        for &n in ns {
            let expected = count_parallel(n);
            for &k in &[1usize, 4, 12] {
                let got = count_parallel_sharded(n, 1, k);
                assert_eq!(
                    got, expected,
                    "K=1 M={k} mismatch at n={n}: got={got} expected={expected}"
                );
            }
        }
    }

    /// Sharded oracle: multi-pool (K=3, M=4) must agree with single-pool.
    /// Exercises stride partitioning and cross-pool summation.
    #[test]
    fn sharded_k3_matches_parallel() {
        let ns: &[usize] = &[0, 4, 16, 32, 48, 60, 80, 100, 120];
        for &n in ns {
            let expected = count_parallel(n);
            let got = count_parallel_sharded(n, 3, 4);
            assert_eq!(
                got, expected,
                "K=3 M=4 mismatch at n={n}: got={got} expected={expected}"
            );
        }
    }

    /// Sharded oracle: pathological extreme (K=12, M=1 — one pool per
    /// thread, no work-stealing within a pool). Must still produce the
    /// exact count; this catches misuse of `with_max_len`, stride math, or
    /// scope-thread join order.
    #[test]
    fn sharded_extreme_matches() {
        let ns: &[usize] = &[0, 16, 48, 80, 120];
        for &n in ns {
            let expected = count_parallel(n);
            let got = count_parallel_sharded(n, 12, 1);
            assert_eq!(
                got, expected,
                "K=12 M=1 mismatch at n={n}: got={got} expected={expected}"
            );
        }
    }
}
